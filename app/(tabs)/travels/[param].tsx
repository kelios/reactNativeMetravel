...
const LazyFallback = () => (
    <View style={styles.fallback}><ActivityIndicator size="small" color="#6B4F4F" /></View>
);
...
export default function TravelDetails() {
    ...
    const [menuOpen, setMenuOpen] = useState(width >= 1024);
    ...
    const scrollTo = useCallback((k: keyof typeof anchor) => {
        const handle = findNodeHandle(anchor[k]?.current);
        if (handle && scrollRef.current) {
            scrollRef.current.scrollTo({
                y: anchor[k]?.current?.measure ? anchor[k].current.measure((x, y) => y - headerOffset) : 0,
                animated: true,
            });
            if (isMobile || isTablet) closeMenu();
        }
    }, [anchor, headerOffset, isMobile, isTablet]);

    const animatedX = useRef(new Animated.Value(width >= 1024 ? 0 : -MENU_WIDTH)).current;
            ...
            <View style={styles.wrapper}>
                <SafeAreaView style={styles.safeArea}>
                    <View style={styles.mainContainer}>
                        <Animated.View style={[styles.sideMenu, { transform: [{ translateX: animatedX }], width: MENU_WIDTH, zIndex: 1000 }]}>
                        ...
                        <ScrollView
                            ...
                            style={[styles.scrollView, { marginLeft: width >= 1024 ? MENU_WIDTH : 0 }]}
                        >
                            <View style={styles.contentOuter}>
                                <View style={styles.contentWrapper}>
                                        ...
                                        <View style={styles.videoContainer}>
                                            {Platform.OS === 'web' ? (
                                                <div style={{ width: '100%', height: '100%' }}>
                                                    <iframe
                                                        src={`https://www.youtube.com/embed/${travel.youtube_link.split('v=')[1]}`}
                                                        width="100%"
                                                        height="100%"
                                                        style={{ border: 'none' }}
                                                        allowFullScreen
                                                ...
                                                <WebView source={{ uri: `https://www.youtube.com/embed/${travel.youtube_link.split('v=')[1]}` }} style={{ flex: 1 }} />
                                            )}
                                        </View>
                                    )}
                                    ...
                                    {[{ ref: anchor.description, html: travel.description, title: travel.name },
                                        ...
                                        { ref: anchor.minus, html: travel.minus, title: 'Минусы' }].map(({ ref, html, title }) => html && (
...
const styles = StyleSheet.create({
    ...
    contentWrapper: {
        ...
        width: '100%',
        maxWidth: 1200,
        paddingHorizontal: 16, // немного паддинга по бокам, чтобы контент "дышал"
    },
    sliderContainer: {
        width: '100%',
        marginBottom: 24,
