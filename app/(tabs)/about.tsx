...
export default function AboutAndContactScreen() {
  ...
  const isWide = width >= 900; // десктоп/широкие экраны → 2 колонки
  ...
  const clearForm = () => {
    setName(''); setEmail(''); setMessage(''); setAgree(false); setTouched({}); setHp('');
  };
  ...
  const handleSubmit = async () => {
    ...
    if (hp.trim()) return; // бот — тихо выходим
    ...
    if (!agree)
      return setResp({ text: 'Требуется согласие на обработку данных.', error: true });
  ...
  return (
      <>
        <Head>
          ...
          <meta key="description" name="description" content="Проект MeTravel — сообщество путешественников. Делитесь маршрутами, пишите статьи, вдохновляйтесь идеями!" />
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
            <ImageBackground source={require('@/assets/images/media/slider/about.jpg')} style={styles.backgroundImage} resizeMode="cover">
                ...
                <View style={styles.content}>
                  ...
                  <View style={isWide ? styles.twoColumns : styles.oneColumn}>
                    ...
                    <View style={isWide ? styles.column : null}>
                      ...
                      <Paragraph style={styles.paragraph}>
                        Проект запущен в июне 2020. Использование материалов — только с разрешения автора. Идеи, отзывы и
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.form}>
                      {response.text !== '' && (
                          <Text style={[styles.response, response.error ? styles.err : styles.ok]}>
                            {response.text}
                          </Text>
                      )}
                      ...
                      <TouchableOpacity
                          activeOpacity={0.7}
                          onPress={() => { setAgree(!agree); setTouched(s => ({ ...s, agree: true })); }}
...
const styles = StyleSheet.create({
  ...
  videoThumbWrap: {
    width: 200, // фиксируем узкую колонку на десктопе
    alignItems: 'center',
  ...
  playBadge: {
    ...
    backgroundColor: 'rgba(0,0,0,0.55)', alignItems: 'center', justifyContent: 'center', position: 'absolute',
  },
  playIcon: { color: '#fff', fontSize: 28, marginLeft: 4 },

  // Правая часть карточки (если нужна подпись)
  ...
  // Мобильное поведение для колонки с видео
  ...
  checkbox: {
    width: 20, height: 20, borderRadius: 4, borderWidth: 1.5, borderColor: '#6AAAAA',
