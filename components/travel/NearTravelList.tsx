...
const SegmentSwitch = ({ value, onChange }: { value: 'list' | 'map'; onChange: (val: 'list' | 'map') => void }) => (
    <View style={segmentStyles.container}>
        <Pressable onPress={() => onChange('list')} style={[segmentStyles.button, value === 'list' && segmentStyles.activeButton]}>
            <Text style={[segmentStyles.text, value === 'list' && segmentStyles.activeText]}>Список</Text>
        </Pressable>
        <Pressable onPress={() => onChange('map')} style={[segmentStyles.button, value === 'map' && segmentStyles.activeButton]}>
            <Text style={[segmentStyles.text, value === 'map' && segmentStyles.activeText]}>Карта</Text>
        </Pressable>
    </View>
);
...
const NearTravelList = memo(({ travel, onLayout, onTravelsLoaded }) => {
    ...
    useEffect(() => {
        if (!isMobile) setVisibleCount(12);
    }, [isMobile]);
    ...
    return (
        <View style={styles.section} onLayout={onLayout}>
            ...
            {isMobile && <SegmentSwitch value={viewMode} onChange={setViewMode} />}
            {!isMobile && (
...
const styles = StyleSheet.create({
    ...
    mapColumn: { flex: 1, height: 500, minWidth: 360 },
    mobileMapWrapper: { height: 400, marginTop: 16, borderRadius: 16, overflow: 'hidden' },
...
const segmentStyles = StyleSheet.create({
    ...
    button: { paddingVertical: 8, paddingHorizontal: 24, backgroundColor: '#fff' },
