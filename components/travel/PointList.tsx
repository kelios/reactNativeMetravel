  ...
  responsive: any;
    ...
    <>
      ...
      <View style={styles.overlay}>
        ...
        {point.description && expanded && <Text style={styles.description}>{point.description}</Text>}
        {point.categoryName && (
...
const PointList: React.FC<PointListProps> = ({ points }) => {
  ...
  const handleSendToTelegram = useCallback((coordStr: string) => {
    ...
    Linking.openURL(url).catch(() => Alert.alert('Ошибка', 'Не удалось открыть Telegram'));
  }, []);
  ...
  const handleOpenMap = useCallback((coordStr: string) => {
    ...
    Linking.openURL(url as string).catch(() => Alert.alert('Ошибка', 'Не удалось открыть карты'));
  }, []);
  ...
  const getCardStyle = (w: number) =>
      w >= 1024 ? { flexBasis: '31%', maxWidth: '31%' } : w >= 600 ? { flexBasis: '48%', maxWidth: '48%' } : { flexBasis: '100%', maxWidth: '100%' };

  const getResponsiveStyles = (w: number) => {
    let minHeight = 260;
    let titleSize = 15;
    let coordSize = 13;
    if (w >= 1024) {
        ...
        {showCoords && (
            ...
            >
              {points.map((point, index) => (
                  <Animated.View key={point.id} entering={FadeIn.delay(index * 30)} style={[styles.card, getCardStyle(width)]}>
                    <Pressable
...
const styles = StyleSheet.create({
  ...
  toggleButton: {
    ...
    paddingVertical: 12, paddingHorizontal: 16, backgroundColor: '#fff', borderRadius: 16, elevation: 2, gap: 8,
    marginBottom: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4,
  },
  toggleButtonPressed: { backgroundColor: '#f5f5f5' },
  toggleText: { fontSize: 16, fontWeight: '600', color: '#3B2C24', fontFamily: 'Roboto-Medium' },
  toggleTextMobile: { fontSize: 14 },
  cardsContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', paddingBottom: 40, gap: 12, paddingHorizontal: 4 },
  card: {
    borderRadius: 12, overflow: 'hidden', backgroundColor: '#f3f3f3', elevation: 3, flexGrow: 1, flexShrink: 0,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, display: 'flex',
  },
  image: { justifyContent: 'flex-end', flex: 1 },
  noImage: { backgroundColor: '#e0e0e0' },
  iconButtons: {
    position: 'absolute', top: 8, right: 8, flexDirection: 'row', zIndex: 2,
    backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: 4,
  },
  iconButton: { marginHorizontal: 2 },
  overlay: { padding: 12, backgroundColor: 'rgba(0,0,0,0.6)', borderBottomLeftRadius: 12, borderBottomRightRadius: 12 },
  title: { color: '#fff', fontWeight: '600', marginBottom: 6, fontFamily: 'Roboto-Medium' },
  coord: { color: '#cceeff', textDecorationLine: 'underline', marginBottom: 8, fontFamily: 'Roboto-Regular' },
  description: { color: '#fff', fontSize: 13, marginBottom: 8, fontFamily: 'Roboto-Regular', lineHeight: 18 },
  categoryContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 4 },
  category: { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 10, paddingHorizontal: 8, paddingVertical: 4, alignSelf: 'flex-start' },
  categoryText: { color: '#fff', fontSize: 12, fontWeight: '500', fontFamily: 'Roboto-Medium' },
});
