...
const QuillEditor = isWeb && win ? (React.lazy(() => import('react-quill')) as any) : undefined;
...
const WebEditor: React.FC<ArticleEditorProps> = ({
    ...
    const Toolbar = () => (
        <View style={styles.bar}>
            ...
            <View style={styles.row}>
                ...
                <IconButton name="code" onPress={() => { tmpStoredRange.current = quillRef.current?.getEditor().getSelection(); setShowHtml(v => !v); }} />
                <IconButton name={fullscreen ? 'fullscreen-exit' : 'fullscreen'} onPress={() => { tmpStoredRange.current = quillRef.current?.getEditor().getSelection(); setFullscreen(v => !v); }} />
                <IconButton name="image" onPress={() => { if (!win) return; const input = win.document.createElement('input'); input.type = 'file'; input.accept = 'image/*'; input.onchange = () => { const file = input.files?.[0]; if (file) uploadAndInsert(file); }; input.click(); }} />
            </View>
        </View>
    );
    ...
    const Loader = () => (<View style={styles.loadBox}><Text style={styles.loadTxt}>Загрузка…</Text></View>);

    if (!QuillEditor) return <Loader />;

    const editorArea = showHtml ? (
        <TextInput style={styles.html} multiline value={html} onChangeText={fireChange} placeholder={placeholder} placeholderTextColor="#999" />
    ) : (
        <Suspense fallback={<Loader />}>
            <QuillEditor ref={quillRef} theme="snow" value={html} onChange={(val: string) => fireChange(val)} modules={quillModules} placeholder={placeholder} style={styles.editor} />
        </Suspense>
    );

    const body = (<>{<Toolbar />}{editorArea}</>);
...
const NativeEditor: React.FC<ArticleEditorProps> = ({
    ...
    return (
        <View style={styles.wrap}>
            ...
            <TextInput multiline value={text} onChangeText={onEdit} placeholder={placeholder} placeholderTextColor="#999" style={styles.html} textAlignVertical="top" />
        </View>
    );
};
...
const styles = StyleSheet.create({
    wrap: { borderWidth: 1, borderColor: '#ddd', borderRadius: 6, backgroundColor: 'var(--bg)', marginVertical: 8 },
    bar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8, backgroundColor: 'var(--bar)', borderBottomWidth: 1, borderBottomColor: '#ddd' },
    label: { fontSize: 15, fontWeight: '600', color: 'var(--fg)' },
    row: { flexDirection: 'row' },
    btn: { marginLeft: 12, padding: 4 },
    editor: { minHeight: 200 },
    html: { minHeight: 200, padding: 12, fontSize: 14, color: 'var(--fg)' },
